...
This kind of workflow should be applicable to every vulnerability type. But I think it works especially well for **SQL injections**, because the potentially vulnerable sinks are quite easy to find using simple keywords, as explained in [section 2.1](#how-joomla-interacts-with-the-database). My guess is that vulnerabilities like **XSS** are harder to find with this workflow because there are many different ways to execute **JavaScript**. On top of that, **JavaScript** code often has many different ways for user-controlled input to end up executing in another user's browser. Another category that aligns well with this AI-assisted workflow is **heap-related vulnerabilities** in application security. Regardless of the underlying heap manager, specific API functions such as glibc's `malloc` and `free` are universally used for allocating and freeing memory chunks. However, performing sink analysis here differs from **SQL injection**. Memory allocation calls are not the final sinks themselves, but rather intermediate steps within the source-to-sink trace. Consequently, the AI agent requires different instructions than the ones designed for **SQL injection** analysis.
